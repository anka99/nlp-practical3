...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(embed_size + hidden_size, hidden_size, bias=True) # todo input layer idk
    ...
    def encode(
        ...
        init_decoder_hidden = torch.cat((last_hidden[0].permute(1, 0), last_hidden[1].permute(1, 0))).permute(1, 0)
        init_decoder_hidden = self.h_projection(init_decoder_hidden)
        
        init_decoder_cell = torch.cat((last_cell[0].permute(1, 0), last_cell[1].permute(1, 0))).permute(1, 0)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # print(Y.shape) TODO why tgt_len == hidden_size ? 
        ...
        for Y_t in Y_ts:
            ...
            dec_state, combined_output, e_t = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks) # (b, e), (b, e)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 3. TODO: Compute the attention scores e_t, a Tensor shape (b, src_len).
        ...
        alpha_t = nn.functional.softmax(e_t, dim=0)
        ...
        a_t = torch.matmul(enc_hiddens.permute(0, 2, 1), alpha_t.unsqueeze(-1)).squeeze(-1)
